...
async function fetchActivity(shiftId: string): Promise<ShiftActivityEvent[]> {
...
export function useShiftActivity(shiftId: string | null | undefined) {
  ...
  const query = useQuery({
    queryKey: shiftId ? qk.shiftActivity.byShift(shiftId) : ['shiftActivity', 'none'],
    queryFn: () => fetchActivity(shiftId as string),
    enabled: !!shiftId,
  });
  ...
  useEffect(() => {
    if (!shiftId) return;
    const invalidate = () => qc.invalidateQueries({ queryKey: qk.shiftActivity.byShift(shiftId) });
    const channel = supabase
      .channel(`shift-activity-${shiftId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'attendance_records', filter: `shift_id=eq.${shiftId}` }, invalidate)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'call_off_requests', filter: `shift_id=eq.${shiftId}` }, invalidate)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'swap_requests', filter: `shift_id=eq.${shiftId}` }, invalidate)
      .subscribe();
